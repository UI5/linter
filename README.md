...
After applying fixes, the linter runs another pass to detect any remaining issues. Not all findings may be fixable, so those issues may need to be addressed manually. See [Migration Limitations](./docs/Migration-Limitations.md) for examples of APIs that cannot be migrated automatically. Note that this list is not exhaustive; there are more APIs that are currently not migrated.
